...
void USSaveGameSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	//Make sure its loaded into memory .Get() only resolves if already loaded previously elsewhere in code
	UDataTable* DummyTable = SGSettings->DummyTablePath.LoadSynchronous();
	//DummyTable->GetAllRows(); //we dont neet this table for anything, just a content reference example
}
...
void USSaveGameSubsystem::WriteSaveGame()
{
	...
	//Iterate the enture world of actors
	for (FActorIterator It(GetWorld()); It; ++It)
	{
		...
		//Pass the array to fill with data with Actor
